...
test "unit: registers block offsets match NVMe controller properties" {
    ...
    try std.testing.expectEqual(@as(usize, 0x0008), @offsetOf(registers.RegisterBlock, "vs"));
    try std.testing.expectEqual(@as(usize, 0x000c), @offsetOf(registers.RegisterBlock, "intms"));
    try std.testing.expectEqual(@as(usize, 0x0010), @offsetOf(registers.RegisterBlock, "intmc"));
    try std.testing.expectEqual(@as(usize, 0x0014), @offsetOf(registers.RegisterBlock, "cc"));
    try std.testing.expectEqual(@as(usize, 0x001c), @offsetOf(registers.RegisterBlock, "csts"));
    try std.testing.expectEqual(@as(usize, 0x0020), @offsetOf(registers.RegisterBlock, "nssr"));
    try std.testing.expectEqual(@as(usize, 0x0024), @offsetOf(registers.RegisterBlock, "aqa"));
    try std.testing.expectEqual(@as(usize, 0x0028), @offsetOf(registers.RegisterBlock, "asq"));
    try std.testing.expectEqual(@as(usize, 0x0030), @offsetOf(registers.RegisterBlock, "acq"));
}

test "unit: registers block size ends at doorbell base" {
    try std.testing.expectEqual(@as(usize, 0x1000), @sizeOf(registers.RegisterBlock));
    try std.testing.expectEqual(registers.doorbell_base_offset, @sizeOf(registers.RegisterBlock));
}

test "unit: registers at rejects short BAR window" {
    var short: [0x0fff]u8 align(@alignOf(u64)) = @splat(0);
    try std.testing.expectError(error.OutOfBounds, registers.ControllerRegisters.at(&short));
}

test "unit: cap decodes queue entries timeout doorbell stride and page sizes" {
    const sample: registers.Cap = .{
        .mqes = 0x00ff,
        .cqr = 0,
        .ams = 0,
        .to = 20,
    ...
    const round = registers.Cap.fromRaw(sample.raw());
...
test "unit: cap detects NVM command set support" {
    ...
    const without_nvm: registers.Cap = .{
        .mqes = 0,
        .cqr = 0,
        .ams = 0,
        .to = 0,
...
test "unit: version decodes major minor tertiary" {
    ...
    const decoded = registers.Version.fromRaw(0x00020100);
...
test "unit: cc nvm enabled encodes NVM CSS and queue entry sizes" {
    const c0 = registers.Cc.nvmEnabled(0);
    try std.testing.expectEqual(@as(u1, 1), c0.en);
    try std.testing.expectEqual(registers.CommandSetSelection.nvm, c0.css);
    try std.testing.expectEqual(@as(u4, 0), c0.mps);
    try std.testing.expectEqual(registers.Arbitration.round_robin, c0.ams);
    try std.testing.expectEqual(registers.ShutdownNotification.none, c0.shn);
    ...
    const c4 = registers.Cc.nvmEnabled(4);
    try std.testing.expectEqual(@as(u4, 4), c4.mps);
    try std.testing.expectEqual(@as(u1, 1), c4.en);
    try std.testing.expectEqual(registers.CommandSetSelection.nvm, c4.css);
    try std.testing.expectEqual(registers.Arbitration.round_robin, c4.ams);
    try std.testing.expectEqual(registers.ShutdownNotification.none, c4.shn);
...
test "unit: cc shutdown notification updates only SHN" {
    const start = registers.Cc.nvmEnabled(0);
    const next = start.withShutdown(.abrupt);
    try std.testing.expectEqual(registers.ShutdownNotification.abrupt, next.shn);
...
test "unit: csts decodes ready fatal and shutdown status" {
    ...
    try std.testing.expectEqual(registers.ShutdownStatus.occurring, a.shst);

    const b: registers.Csts = .{
        .rdy = 0,
        .cfs = 1,
        .shst = .complete,
        .nssro = 0,
    ...
    try std.testing.expectEqual(registers.ShutdownStatus.complete, b.shst);
}

test "unit: aqa encodes named one-based depths as zero-based fields" {
    const one = try registers.Aqa.fromDepths(.{ .submission_entries = 1, .completion_entries = 1 });
    try std.testing.expectEqual(@as(u12, 0), one.asqs);
    try std.testing.expectEqual(@as(u12, 0), one.acqs);

    const max = try registers.Aqa.fromDepths(.{ .submission_entries = 4096, .completion_entries = 4096 });
    try std.testing.expectEqual(@as(u12, 0xfff), max.asqs);
    try std.testing.expectEqual(@as(u12, 0xfff), max.acqs);

    const asymmetric = try registers.Aqa.fromDepths(.{ .submission_entries = 64, .completion_entries = 128 });
...
test "unit: aqa rejects zero and too-large depths" {
    try std.testing.expectError(
        ...
        registers.Aqa.fromDepths(.{ .submission_entries = 0, .completion_entries = 64 }),
    );
    try std.testing.expectError(
        error.QueueDepthOutOfRange,
        registers.Aqa.fromDepths(.{ .submission_entries = 64, .completion_entries = 0 }),
    );
    try std.testing.expectError(
        error.QueueDepthOutOfRange,
        registers.Aqa.fromDepths(.{ .submission_entries = 4097, .completion_entries = 64 }),
    );
    try std.testing.expectError(
        error.QueueDepthOutOfRange,
        registers.Aqa.fromDepths(.{ .submission_entries = 64, .completion_entries = 4097 }),
    );
}
